...
| Interpolation | `LinearInsert`, `LagrangeInsert`, `NewtonInsert_*`, `CubicSpline`, `Hermite` | `src/Insert.hpp` | Not yet migrated in M3; depends heavily on `QinJiuShao` | node handling and polynomial assembly | strict input validation, no legacy matrix dependency for spline solve | none yet | Indirectly benefits from migrated polynomial ops | legacy characterization only | planned |
| Scalar iteration | `Bisection`, `Picard`, `Newton`, `Secant` | `src/Iteration.hpp` | Loops are reviewable; current API mixes exceptions and old result structs | bisection/Picard/Newton/Secant iteration formulas | unified status/result, finite checks, derivative-zero handling | none yet | No | legacy inventory only | planned |
