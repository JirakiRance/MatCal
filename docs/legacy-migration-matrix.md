...
| Gaussian elimination | `solve_columnElimination` | `src/Matrix.hpp` | M7 routes legacy facade through the structured dense partial-pivot solver | row-pivot elimination | multi-RHS factor reuse future work | `MatCal::Linalg::solve_dense_partial_pivot` | Yes for `solve_columnElimination` | legacy characterization, adapter differential, linalg oracle | migrated |
...
| Jacobi / Gauss-Seidel / SOR | `Jacobi`, `Gauss_Seidel`, `SOR` | `src/Matrix.hpp` | M7 migrates stationary solvers to structured Linalg core | Jacobi/GS/SOR update formulas, corrected SOR omega formula | advanced preconditioning/sparse iteration future work | `MatCal::Linalg::solve_jacobi`, `solve_gauss_seidel`, `solve_sor` | Yes | convergence, scale, non-convergence, legacy differential | migrated |
| Eigenvalue | power / inverse power methods | `src/Matrix.hpp` | M7 migrates power and shifted inverse power to Linalg | largest-component normalization, shift-invert idea | clustered/repeated eigenvalue robustness, full eigensolver future work | `MatCal::Linalg::dominant_eigenpair`, `inverse_power_eigenpair` | Yes | diagonal/symmetric/negative/shifted/residual/legacy differential | migrated |
...
| Differentiation | `Derivative::*` | `src/Basics.hpp` | M5 migrates scalar helpers; M7 migrates multivariable partial/gradient helpers | forward and centered finite-difference formulas | no automatic step selection | `MatCal::Calculus` | Yes | oracle, invalid input, legacy differential | migrated |
...
## M7 Completed Migration

`MatCal::Legacy` now has a single conversion layer for legacy matrix/vector data copied into `MatCal::Linalg::DenseMatrix` and `Vector`, plus conversion back to legacy `Matrix`.

`MatCal::Linalg` now owns stationary Jacobi, Gauss-Seidel, and SOR solvers. Legacy `Jacobi`, `Gauss_Seidel`, and `SOR` delegate to these functions.

`MatCal::Linalg` now owns dense reference power and shifted inverse-power eigen solvers. Legacy `PowerMethod` and `PowerMethod_reverse` delegate to them.

Legacy `solve_columnElimination` delegates to `solve_dense_partial_pivot`. Legacy `LU_Decompose` remains the documented no-pivot LU compatibility implementation.

`MatCal::Calculus` now owns multivariable forward partial derivatives and gradients. Legacy `Derivative::pF_px` and `Derivative::dF_dx` delegate to this core.
